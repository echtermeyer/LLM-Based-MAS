...
($\tau = 0.4$). Temperature controls the entropy of the output distribution: lower
...
Empirically, ablating temperature from $\tau = 0.4$ to $\tau = 0.7$ in a comparable
...
\paragraph{Task-selection bias from outcome-variance screening.}
All quantitative results are conditioned on tasks selected by the screening
procedure described in \autoref{sec:setup:design}: those where the outcome
...
conditions where most tasks fall outside this regime, would overstate motif frequency.
...
(\autoref{sec:results:evid}). This is a real restriction on the causal claim,
...
A devil's advocate probe (\autoref{sec:results:evid}) suggests this is a
