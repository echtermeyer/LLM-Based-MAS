...
What these treatments share is a substrate in which no option is better than
any other: conventions, arbitrary labels, or non-empirical propositions.
They can show that a group locks in, but not whether it locks in on the right
answer, because there is no right answer. And none of them measure whether the
detected patterns exceed what chance alignment in a small discrete state space
would produce, because no principled null is needed when correctness is not
defined. Both gaps remain open, and \citet{mehdizadeh2026topologymemory} name
them explicitly as a critical open question.
