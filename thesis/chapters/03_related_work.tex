...
directions depending on where an agent sits. Faster settling in the centralised
...
and influence one another. Each of the four motifs we detect has a
correspondingly documented pathology, and we take them in turn.
...
answers rather than challenge flawed arguments; they attribute this to
sycophancy \citep{sharma2023sycophancy}. \citet{cho2025herd} find the gap
...
diffusing, outcomes come to depend heavily on the competence of whichever agent
occupies the centre \citep{conformitydynamics2026}, a dependency our star
topology is designed to expose.
...
premature agreement. A structurally sharper version is captured by HiddenBench
...
majority rarely recovers. In every case the opening distribution is observed as
it arises rather than constructed, leaving its effect confounded with task
difficulty and agent confidence, which is what \autoref{sec:results:seeded} is
designed to separate.

\medskip
\noindent The vocabulary needed to name these phenomena precisely comes from
complex-systems theory. Recurring interaction structures are studied as
\emph{network motifs}, small subgraphs appearing more often than in a randomised
network and generating characteristic behaviour \citep{milo2002network,
alon2007network}; the complementary notion of a \emph{dynamical motif}, a
recurring pattern in the trajectory such as an attractor or an oscillation, has
recently been formalised for recurrent networks \citep{driscoll2024dynamical}.
The individual regimes are classical: positive feedback and lock-in
\citep{arthur1989competing}, and limit cycles, fixed points, and multistability
in nonlinear dynamics \citep{strogatz2015nonlinear}. On the influence side,
opinion-dynamics models formalise how agreement and social power propagate and
when influence concentrates on a single node \citep{degroot1974consensus,
jia2015opinion}, and the continuous-opinion, discrete-action framework
\citep{martins2008coda} matches agents that hold latent confidence but cast
discrete votes. The structural topology a designer imposes is moreover not the
influence structure that forms, a distinction long standard in network
neuroscience as structural versus effective connectivity
\citep{friston2011connectivity} and one we take up in
\autoref{sec:results:eff_topology}.

That this vocabulary applies to LLM populations is established.
...
\citep{centola2018tipping}. That it can be made analytic is established too:
\citet{okawa2026biasedconsensus} derive a mean-field treatment of debate from
...
Neither setting, however, has a correct answer for the collective to converge
on, a limitation we return to in \autoref{sec:related:gap}. At the level of
principle, \citet{miehling2025agentic} argue that agentic AI must be analysed
through a systems-theoretic lens precisely because group-level behaviour cannot
be predicted from individual agent properties, but their contribution is
conceptual and supplies no empirical instrument.
...
information-theoretically through transfer entropy over execution traces
\citep{caspian2026, infodynamics2026}, or by treating influence as a cooperative
...
explicitly to avoid convergence driven by factual correctness, and
\citet{okawa2026biasedconsensus} uses symbols with no correct answer. This is a
...
whether it locks in on the right answer.
\citet{mehdizadeh2026topologymemory} say as much themselves, naming as a
critical open question whether their findings survive in epistemic settings
where the symmetry between options breaks because some claims are better
...
detectors for self-reinforcement, bistability, dominance, and limit cycles, each
