...
observe. This section specifies that instrument: the system model and notation
...
quantitatively; the influence graph for dominance detection is built from vote
adoptions during the \updatephase{} phase (\autoref{sec:method}), not from the draft text.
...
The system's answer for a repetition is the \emph{plurality} of the agents' votes
...
cap $T = 15$ without converging (for instance the two-way $2$--$2$ split of a limit
cycle); such ties are broken deterministically towards the first option in
...
One structural detail of HiddenBench matters for the analysis:
$N = 4$ agents throughout, while the number of answer options is task-dependent,
...
at all, so they are promoted to a deep \emph{characterisation} pass on the full
...
GPQA-Diamond yields more than $50$ Tier-3 tasks in the screening pass, so we
retain $50$ of them to match the number of eligible HiddenBench tasks, giving a
balanced design with equal task counts per benchmark; the $50$ are drawn
