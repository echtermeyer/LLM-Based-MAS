...
bias, and round-$0$ entries omit the argument field, which does not exist yet. A larger memory window
...
shrink. Analysing pilot repetitions (the efficiency trade-off is detailed in Appendix~\ref{app:knobs}), we find that the
...
variable; expert validation gives unambiguous ground truth; at release, frontier
models were well below ceiling \citep{rein2024gpqa}, and while the most capable
models have since reached $\sim$85\% \citep{becker2025orchestration}, mid-tier
models remain well below, leaving genuine headroom for debate.
GPQA-Diamond also has direct multi-agent precedent
\citep{becker2025orchestration, kenton2024debate, wang2025mars}. We
...
\begin{itemize}
  \item \textbf{Memory window} $W \in \{1, 2, 5\}$, with $W = 1$ the near-Markov
        ...
        \citep{du2024improving, mehdizadeh2026topologymemory, marandi2026protocols}.
...
\begin{itemize}
  ...
  \item \textbf{Sampling} temperature $0.7$, reasoning disabled: diverse yet
        focused outputs; for homogeneous agents stochasticity arises solely from
        sampling \citep{zhu2026demystifying}, so a positive temperature is necessary
        for any response diversity.
...
\midrule
Memory $W$ & scan & $\{1, 2, 5\}$ & \citep{du2024improving, mehdizadeh2026topologymemory, marandi2026protocols} \\
...
Synchrony & fix & parallel-synchronous & \citep{du2024improving, chan2024chateval} \\
...
A further interpretive consequence concerns multistability. Because tasks are
...
\autoref{sec:results:multistability} is conditioned on this selection and should not
