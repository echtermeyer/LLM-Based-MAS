...
$\text{draft}_i^{t}$ & \arguephase{} draft of agent $i$ (defense, challenge, question) \\
...
$\mathcal{H}^{\text{self}}_i(t,W)$ & self history window of agent $i$ (own recent states) \\
$\mathcal{H}^{\text{peer}}_i(t,W)$ & peer history window of agent $i$ (neighbours' recent states) \\
...
\begin{itemize}
  \item \textbf{Fully connected (fc):} $A_{ij} = 1 - \delta_{ij}$, so every agent
        ...
        serves as the baseline. With $N = 4$ agents (GPQA-Diamond), each round involves
        $N(N-1) = 12$ messages.
  \item \textbf{Star:} one hub agent is connected to all $N-1$ spokes, and the
        ...
        Each round involves $2(N-1) = 6$ messages (GPQA-Diamond), half the fully-connected cost.
        Star therefore trades per-round token cost for slower convergence: the hub
...
number of agents $N$ and the number of options $M$ are independent: $N = 4$ throughout, whereas $M$ varies by task. The composition retains the information a
...
\paragraph{\arguephase{} draft.}
For rounds $t \geq 1$ each agent also emits an ephemeral \emph{draft} before
committing to a belief:
\begin{equation}
  \text{draft}_i^{t} = \big(\text{defense}_i^{t},\ \text{challenge}_i^{t},\ \text{question}_i^{t}\big),
\end{equation}
...
draft is visible to neighbours at the start of the \updatephase{} phase but does not itself change
...
\arguephase{} phase completes before the \updatephase{} phase begins. Round $0$ is asymmetric, as no
...
All outputs are elicited as structured JSON with schema enforcement via function
calling, guaranteeing the required fields in every response; the rare malformed
response triggers a single retry before the round is logged as a skip.
The complete prompt templates are reproduced verbatim in \autoref{app:prompts}.
...
% r=0 --- Phase B only
\node[phB] (B0i1) at (0, 0) {
  \textbf{Agent $1$ \textbullet\ \updatephase{} phase}\\[3pt]
  $v_1^{0}$: vote\\[1pt]
...
\node[phB] (B0i2) at (7.4, 0) {
  \textbf{Agent $2$ \textbullet\ \updatephase{} phase}\\[3pt]
  $v_2^{0}$: vote\\[1pt]
...
\node[phA, below=0.7cm of B0i1] (Ar1i1) {
  \textbf{Agent $1$ \textbullet\ \arguephase{} phase}\\[3pt]
  $\text{draft}_1^{1}$: defense, challenge, question
};
\node[phA, below=0.7cm of B0i2] (Ar1i2) {
  \textbf{Agent $2$ \textbullet\ \arguephase{} phase}\\[3pt]
...
\node[draw, rounded corners=3pt, fill=gray!5, align=left, inner sep=6pt]
  ...
  {\color{orange!80}$\longrightarrow$}~$\text{draft}$: \arguephase{} phase output, sent to
...
$t{=}0,1,2$. Round~$0$ has no \arguephase{} phase: agents read
...
round's \arguephase{} phase; an \arguephase{} draft reaches neighbours in the \emph{same} round's
...
bias \citep{zheng2024selectors}, and round-$0$ entries omit the draft field, which does not exist. A larger
...
\begin{itemize}
  ...
  \item \textbf{Calibrated difficulty:} frontier models fall well below ceiling
        \citep{rein2024gpqa, becker2025orchestration},
...
a mid-tier model (\texttt{mistral-medium-instruct}) on the promoted Tier-3 subset only.
...
Two structural details of HiddenBench matter for the analysis. First,
...
$M$ options.
Extension to mixed-motive negotiation settings is left to future work.
...
\begin{itemize}
  ...
  \item \textbf{Model} \texttt{mistral-medium-instruct} (homogeneous): a single mid-tier
